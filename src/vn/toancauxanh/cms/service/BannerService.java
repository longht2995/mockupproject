...
public class BannerService extends BasicService<Banner> {
	...
	public Banner getBannerLocation() {
		Banner banner = find(Banner.class)
				.where(QBanner.banner.trangThai.ne(core().TT_KHONG_AP_DUNG))
				.where(QBanner.banner.locationBanner.eq(core().BANNERBODY1))
				.fetchFirst();
		return banner;
	}
